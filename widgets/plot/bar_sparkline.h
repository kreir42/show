//bar_sparkline: a scrolling history of the last w samples, one column per sample, drawn as vertical eighth-block bars
...
static void bar_sparkline_draw(struct widget* widget, char* rowbuf, int* col_eighths, double* samples, struct plot_region pr, struct plot_data* data, double col_seconds){
	int w = pr.w, h = pr.h;
	double lo, hi;
...
void* bar_sparkline(void* input){
	struct widget* widget = input;
	struct plot_data* data = widget->data;
	int h, w;
	get_size(widget, &h, &w);
	struct plot_region pr = plot_layout(data->flags, h, w);
	PLOT_HISTORY_ALLOC(block, samples, col_eighths, rowbuf, pr.w);
	pthread_cleanup_push(free, block); //free on thread cancel
	if(widget->time>0){ //time==0 doesn't make sense for a bar_sparkline
		while(1){
			plot_history_push(samples, pr.w, plot_sample(data->source));
			bar_sparkline_draw(widget, rowbuf, col_eighths, samples, pr, data, widget->time); //each column spans widget->time seconds, so the X axis can be labeled
			sleep(widget->time);
		}
	}
...
//like bar_sparkline, but the source is launched once and each line it prints pushes a sample. widget->time is ignored
void* bar_sparkline_live(void* input){
	struct widget* widget = input;
	struct plot_data* data = widget->data;
	int h, w;
	get_size(widget, &h, &w);
	...
	if(res.fp){
		...
		bar_sparkline_draw(widget, rowbuf, col_eighths, samples, pr, data, 0); //empty plot until the first value arrives; cadence is unknown so no X labels
		char line[64];
		while(fgets(line, sizeof(line), res.fp)!=NULL){
			plot_history_push(samples, pr.w, strtod(line, NULL));
			bar_sparkline_draw(widget, rowbuf, col_eighths, samples, pr, data, 0);
		}
...
//like bar_sparkline, but source is a file path; plots the file's last pr.w lines, redrawn whenever its mtime changes (polled every widget->time seconds)
void* bar_sparkline_file(void* input){
	struct widget* widget = input;
	struct plot_data* data = widget->data;
	int h, w;
	get_size(widget, &h, &w);
	...
	if(widget->time>0){
		...
		while(1){
			if(stat(data->source, &st)==0 && st.st_mtime>last_mtime){
				...
				bar_sparkline_draw(widget, rowbuf, col_eighths, samples, pr, data, 0); //file lines aren't a fixed time interval, so no X labels
			}
			sleep(widget->time);
		}
	}else{
