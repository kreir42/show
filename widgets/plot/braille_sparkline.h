//braille_sparkline: the same rolling time-series as bar_sparkline, but drawn as a connected line on a braille dot canvas. a cell is 2x4 dots, and we use both dot-columns: the history holds 2 samples per cell (2*w samples, twice the horizontal resolution of the other sparklines). since samples are exactly one dot-column apart there is no room to interpolate, so each column is just filled vertically to the previous sample's level. shares the scaling, axes and poll/live/file flavours; the sample buffer and draw differ
...
//render the sample history as a braille line into pr. samples holds 2*pr.w values. cells is scratch of pr.w*pr.h bytes. rowbuf is pr.w*3+1 bytes. col_seconds is the time each sample spans (0 disables X labels)
static void braille_sparkline_draw(struct widget* widget, char* rowbuf, unsigned char* cells, double* samples, struct plot_region pr, struct plot_data* data, double col_seconds){
	...
	for(int c=0; c<n; c++){
		...
		//samples are one dot-column apart, so connecting is just filling this column from the previous sample's level to this one (a lone point fills [y,y])
		braille_vline(cells, w, c, have_prev ? py : y, y);
		py = y; have_prev = 1;
	}
...
void* braille_sparkline(void* input){
	...
	if(widget->time>0){ //time==0 doesn't make sense for a time series
		while(1){
			...
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, widget->time); //each sample spans widget->time seconds, so the X axis can be labeled
			sleep(widget->time);
		}
	}
...
//like braille_sparkline, but the source is launched once and each line it prints pushes a sample. widget->time is ignored
void* braille_sparkline_live(void* input){
	struct widget* widget = input;
	struct plot_data* data = widget->data;
	int h, w;
	get_size(widget, &h, &w);
	...
	if(res.fp){
		...
		braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0); //empty plot until the first value arrives; cadence is unknown so no X labels
		char line[64];
		while(fgets(line, sizeof(line), res.fp)!=NULL){
			plot_history_push(samples, 2*pr.w, strtod(line, NULL));
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0);
		}
...
//like braille_sparkline, but source is a file path; plots the file's last 2*pr.w lines, redrawn whenever its mtime changes (polled every widget->time seconds)
void* braille_sparkline_file(void* input){
	struct widget* widget = input;
	struct plot_data* data = widget->data;
	int h, w;
	get_size(widget, &h, &w);
	...
	if(widget->time>0){
		...
		while(1){
			if(stat(data->source, &st)==0 && st.st_mtime>last_mtime){
				...
				braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0); //file lines aren't a fixed time interval, so no X labels
			}
			sleep(widget->time);
		}
	}else{
