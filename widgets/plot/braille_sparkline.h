//braille sparklines: the same rolling time-series as bar_sparkline, plotted on a braille dot canvas. a cell is 2x4 dots, and we use both dot-columns: the history holds 2 samples per cell (2*w samples, twice the horizontal resolution of the other sparklines). two render styles share all the machinery here, differing only in whether consecutive samples are joined:
...
//both share the scaling, axes and poll/live/file flavours; only the sample buffer and this draw differ from the other sparklines
...
//render the sample history into pr. samples holds 2*pr.w values. cells is scratch of pr.w*pr.h bytes. rowbuf is pr.w*3+1 bytes. col_seconds is the time each sample spans (0 disables X labels). connect!=0 joins consecutive samples (line), connect==0 plots lone dots (scatter)
static void braille_sparkline_draw(struct widget* widget, char* rowbuf, unsigned char* cells, double* samples, struct plot_region pr, struct plot_data* data, double col_seconds, int connect){
	...
	for(int c=0; c<n; c++){
		...
		//line: fill this column from the previous sample's level to this one. scatter (or a lone point): just the single dot at [y,y]
		braille_vline(cells, w, c, (connect && have_prev) ? py : y, y);
		py = y; have_prev = 1;
	}
...
void* braille_line_sparkline(void* input){
	...
	if(widget->time>0){ //time==0 doesn't make sense for a time series
		while(1){
			...
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, widget->time, 1); //each sample spans widget->time seconds, so the X axis can be labeled
			sleep(widget->time);
		}
	}
...
void* braille_line_sparkline_live(void* input){
	...
	if(res.fp){
		...
		braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 1); //empty plot until the first value arrives; cadence is unknown so no X labels
		char line[64];
		while(fgets(line, sizeof(line), res.fp)!=NULL){
			plot_history_push(samples, 2*pr.w, strtod(line, NULL));
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 1);
		}
	...
	if(widget->time>0){
		...
		while(1){
			if(stat(data->source, &st)==0 && st.st_mtime>last_mtime){
				...
				braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 1); //file lines aren't a fixed time interval, so no X labels
			}
			sleep(widget->time);
		}
	}else{
...
void* braille_scatter_sparkline(void* input){
	...
	if(widget->time>0){ //time==0 doesn't make sense for a time series
		while(1){
			plot_history_push(samples, 2*pr.w, plot_sample(data->source)); //2*pr.w samples, one per dot-column
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, widget->time, 0); //connect=0: scatter
			sleep(widget->time);
		}
	}
	pthread_cleanup_pop(1); //frees block, balances the push macro
...
void* braille_scatter_sparkline_live(void* input){
	...
	if(res.fp){
		pthread_cleanup_push(plot_live_cleanup, &res); //kill/reap the child on cancel
		braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 0); //empty plot until the first value arrives; cadence is unknown so no X labels
		char line[64];
		while(fgets(line, sizeof(line), res.fp)!=NULL){
			plot_history_push(samples, 2*pr.w, strtod(line, NULL));
			braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 0);
		}
		while(1) pause(); //command exited: keep the final frame until cancelled
		pthread_cleanup_pop(1); //unreachable, balances the push macro
	}
	...
	if(widget->time>0){
		...
		while(1){
			if(stat(data->source, &st)==0 && st.st_mtime>last_mtime){
				last_mtime = st.st_mtime;
				plot_read_tail(data->source, samples, 2*pr.w); //refill the whole buffer (2 samples per cell) from the file's tail
				braille_sparkline_draw(widget, rowbuf, cells, samples, pr, data, 0, 0); //file lines aren't a fixed time interval, so no X labels
			}
			sleep(widget->time);
		}
	}else{
